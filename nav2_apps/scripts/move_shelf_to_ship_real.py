...
def load_shelf_real():
    """Function to get under the shelf and lift it on the real robot using the approach service from Checkpoint 9"""
    # Create a node for calling the approach service
    node = rclpy.create_node('shelf_loader_real')
...
def unload_shelf_real():
    """Function to lower the shelf and move out from under it on the real robot"""
    # Create nodes for elevator control and velocity
    node = rclpy.create_node('shelf_unloader_real')
...
def update_robot_footprint(with_shelf):
    ...
    node = rclpy.create_node('footprint_updater_real')
